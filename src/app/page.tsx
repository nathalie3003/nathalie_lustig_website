...
export default async function HomePage() {
  ...
  // repeated in the list directly beneath it. The rest are capped at five: the
  // homepage is a front page, not the archive, and "All notes" is one click
  // away. The category tabs are built from whatever lands in this slice, so a
  // tab never offers a filter the visible rows cannot satisfy.
  const rows: HomeNote[] = notes.slice(1, 6).map((n) => ({
    id: n._id,
    slug: n.slug,
    title: n.title,
    excerpt: n.excerpt,
